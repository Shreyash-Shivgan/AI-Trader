...
class ScannerService:
    ...
    def start(self) -> None:
        if not settings.scheduler_enabled:
            ...
            return
        # Free TwelveData plan: 8 credits/min.
        # Scan every 5 minutes (300s) to avoid hitting rate limits.
        self.scheduler.add_job(self.scan_all, "interval", seconds=300, id="scan_all")
        self.scheduler.start()
        logger.info("ScannerService started (interval=300s)")
    ...
    def scan_all(self) -> None:
        ...
        # Only scan on one primary timeframe to conserve API credits.
        # Free TwelveData plan allows 8 requests/min; scanning all timeframes
        # would blow through the quota instantly.
        primary_timeframe = "5m"
        for symbol in symbols:
            try:
                analysis = _run_analysis_sync(symbol, primary_timeframe)
                review = analysis.get("trade_review", {})
                if review.get("allowed_to_alert"):
                    db = SessionLocal()
                    try:
                        signal = _persist_signal(db, symbol, primary_timeframe, analysis)
                ...
                logger.exception("DB error while scanning %s %s", symbol, primary_timeframe)
            except Exception:
                logger.exception("Unexpected error scanning %s %s", symbol, primary_timeframe)
            # 15s delay between symbols to stay within rate limits
            time.sleep(15)
